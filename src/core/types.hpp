...
struct AccountInfo {
  float balance = 0.f;        // "Disp" / Wallet balance
  float pnl = 0.f;            // Unrealized PNL
  int   open_trades = 0;
  int   pending_trades = 0;
};
...
enum class AppTab      { MARKETS, TRADES, BOTS, STRATEGIES, BACKTESTING, EXCHANGES, ACCOUNT, CONFIGURATION };

// Borderless window resize direction
enum class ResizeDir   { NONE, N, S, E, W, NE, NW, SE, SW };

// Window snap zones (Aero Snap emulation for borderless window)
enum class SnapZone    { NONE, TOP, LEFT, RIGHT };
